...
| Variable      | Meaning  |
|---------------|:-------------|
| Breed         | American Kennel Club standard breeds |
| Classification      | Breed Intelligence Category |
| obey | Probability that the breed obeys the fuirst command (figure is lower bound) |
| reps_lower      |  Lower limit of repetitions to understand new commands   |
| reps_upper      |  Upper limit of repetitions to understand new command   |
| height_low_inches      | Height in inches - lower limit |
| height_high_inches      | Height in inches - upper limit |
| weight_low_lbs      | Weight in pounds - lower limit  |
| weight_high_lbs      |  Weight in pounds - upper limit  |
| Heterozygosity      |  Two copies of different alleles   |
| Heterozygosity_x10_4      |  Two copies of different alleles   |
"
cat(tabl)
```
...
```{r}
# Plot of Obey Percentage and Breed color coded
ggplot(combined_complete, aes(x = Breed, y = avg.weight, color = Classification)) +
